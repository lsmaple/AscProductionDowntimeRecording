...
private void Page_Load(object sender, System.EventArgs e)
{
    ...
    string workCenter = Page.Request.Params["WorkCenter"];
    string shiftStart = Page.Request.Params["ShiftStart"];
    DateTime dtShiftStart = Convert.ToDateTime(shiftStart);
        ...
        SPSecurity.RunWithElevatedPrivileges(delegate()
        {
            using (SPSite site = new SPSite(SPContext.Current.Site.ID))
            {
                ...
                using (SPWeb web = site.AllWebs[SPContext.Current.Web.ID])
                {
                    ...
                    SPList list = web.Lists["Production Unscheduled Downtime"];
                    ...
                    oQuery.Query = string.Format(@"
						<Where>
							<And>
								<Eq><FieldRef Name='WorkCenter'/><Value Type='Text'>{0}</Value></Eq>
								<Eq><FieldRef Name='ShiftStart'/><Value Type='DateTime' IncludeTimeValue='TRUE'>{1}</Value></Eq>
							</And>
						</Where>
					", workCenter, SPUtility.CreateISO8601DateTimeFromSystemDateTime(dtShiftStart));
                    SPListItemCollection collListItems = list.GetItems(oQuery);
                    if (collListItems == null) throw new ApplicationException("collListItems is null");
                    DataTable dtItems = collListItems.GetDataTable();
